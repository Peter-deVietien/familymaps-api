...
| 2026-08-19 | NOAA nClimGrid | Added `scripts/download_nclimgrid.py` (resumable, 8 workers, temp-file writes). Pulled 1,080 county CSVs (tmax/tmin/prcp × 1995–2024, ~1 GB) in 85 s. Chosen over PRISM/gridMET because NCEI publishes county-polygon area averages directly, so no zonal statistics were needed. | ✅ 0 failures |
| 2026-08-19 | NOAA nClimGrid | **Caught a silent GEOID corruption.** NCEI county codes are not FIPS — states are numbered 01–48 alphabetically without AK/HI, so `04013` is Contra Costa CA, not Maricopa AZ. Alabama is `01` in both schemes, so the initial spot check falsely confirmed the codes were FIPS. Fixed by rebuilding GEOIDs from the state abbreviation in the name field + trailing 3 digits, with DC special-cased (`18511` → `11001`). Verified by matching county *names* against `US_county_percentages.json`. | ✅ Documented in learnings/data-quirks.md |
| 2026-08-19 | NOAA nClimGrid | Added `scripts/compute_pleasant_days.py`. Counts days/yr with high 60–85°F, low 40–68°F, precip < 0.254 mm, normalised as `pleasant/valid × 365.25`. The 68°F ceiling on the overnight low is a deliberate humidity proxy — nClimGrid has no dewpoint, and without it a muggy Gulf night scores like a dry Mountain West one. | ✅ 3,107 counties; min 18.5 (Lake CO), median 79.8, max 254.2 (Orange CA) |
| 2026-08-19 | NOAA nClimGrid | Added `scripts/add_pleasant_days_field.py` — merges `pleasant_days` into `app/data/US_county_percentages.json`, idempotent, `--dry-run`, warns on orphan GEOIDs. Crosswalks Connecticut's 9 post-2022 planning regions onto their dominant legacy counties (CT spans 74.8–89.6 days, too wide for a statewide average) and Lexington city VA onto Rockbridge County. | ✅ 3,109/3,222 populated; nulls are AK, HI, PR |
| 2026-08-19 | Wiki | New `data/nclimgrid.md`, indexed. Updated data-quirks, api-contract. | ✅ |
| 2026-08-19 | Caching | **Root-caused the rank map going black on phones.** Endpoints sent ETag/Last-Modified but no `Cache-Control`, so browsers used the RFC 9111 heuristic (~10% of file age) and returning visitors served pre-`pleasant_days` JSON for days without revalidating. Added `app/caching.py` with `public, max-age=0, must-revalidate` plus real `If-None-Match` handling across geo, demographics and births. Three silent traps: bare `FileResponse` never compares ETags (that lives in `StaticFiles`), it defers `stat()` so `stat_result=` must be passed or the ETag header is absent at comparison time, and Cloudflare weakens ETags to `W/"..."` on compression so the prefix must be normalised. | ✅ Verified live through CF: match→304/0, weak→304/0, stale→200/580035 |
...
| 2026-08-19 | CDC PLACES | Frontend now paints `us_counties_obesity` as its own layer (crude `obesity_pct`) and uses that same field as the 8th WDWWA rank factor (`reverse: true`). Join stays on the frontend; the percentages JSON is unchanged. | ✅ |

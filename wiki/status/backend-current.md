...
- ✅ **County obesity dataset (2026-08-19)** — new `GET /api/demographics/us_counties_obesity` serving CDC PLACES adult obesity (BMI ≥ 30, adults 18+), crude and age-adjusted with 95% CIs. 3,144 of 3,222 counties populated; nulls are all Puerto Rico. Coalesces two PLACES releases because the newest one omits Kentucky and Pennsylvania entirely. Frontend paints it as its own layer **and** uses crude `obesity_pct` as the 8th WDWWA factor (`reverse: true`). Dataset stays separate from `us_counties_percentages`; the join happens in the frontend. Full details in `data/cdc_places.md`.
